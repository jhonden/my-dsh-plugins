...
- **图片文件查看**：树中点击 png/jpg/gif/webp/bmp/ico/svg，查看器经同一沙箱路由渲染
- **Markdown 内相对路径图片可显示**：`![](img.png)` 引用经沙箱化的 Host 图片路由解析（限定工作区、扩展名白名单、同源校验）
- **只读查看 + 行号 + 语法高亮**：任意文本文件走平台 ReadBlock（shiki 语法、懒加载），显示字节数与截断提示

![语法高亮](docs/screenshots/syntax-highlight.png)
- **文件名筛选框**：对已加载目录做大小写不敏感的子串过滤并高亮命中；目录保留作为通往深层匹配的路径
- **应用内文件链接**：点击聊天工具卡（read 卡、文件改动卡）中的文件路径，直接在文件视图打开，不再唤起本地软件
- **查看器多 tab**：可同时打开多个文件以 tab 切换；每个 tab 记住自己的预览/源码状态；重复打开激活已有 tab；右键 tab 可关闭其他/左侧/右侧
- **递归文件搜索**：过滤框防抖触发全工作区搜索（Remote 方法 + 内置 ripgrep，大小写不敏感、按修改时间新→旧）。结果分两段展示——淡色压缩目录（中省略号保尾部）+ 永不截断的文件名并高亮命中；点击打开，悬停看完整路径
