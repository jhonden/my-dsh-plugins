...
2. An override seam on `openFile` (currently hardcoded to
   ...
   chat/diff cards opens in-app. (A capture-phase DOM interception was tried and withdrawn:
   activating the target view from outside the slot system has no reliable
   seam, so the experience degraded to not switching — the upstream seam is
   the real fix.)
